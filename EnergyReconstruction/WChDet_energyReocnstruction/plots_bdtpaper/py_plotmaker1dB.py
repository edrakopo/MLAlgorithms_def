...
rfile3 = ROOT.TFile('/Users/edrakopo/work/work_energy_reco/keras_reco_TrainTestSamplePAPER.root') #Keras

regTree3=rfile3.Get("tuple")
arr_hi_E3 = tree2array(regTree3,selection="trueKE>0")#200 && trueKE1<600")# && (recoDwall*550.>100. && recoToWall*2500.>100.)")
trueKE3 = arr_hi_E3['trueKE']
recoKE_keras = arr_hi_E3['recoKE']

rfile1 = ROOT.TFile('/Users/edrakopo/work/work_energy_reco/plots_bdtpaper/FINALTMVARegApp_MLP_ener_lilia_CCQE_1040_1100_NEWlookupsTEST_MYselectedVars_large_statTEST10000treesBTEST_14nodes.root') #TMVA MLP
...
recoKE_TMVAMLP = arr_hi_E1['recoKE']
...
#for i in range(0,10):
    if trueKE[i]>E_l and trueKE[i]<E_h and recoDwall[i]*550.>dwall_thres and recoToWall[i]*2500.>towall_thres:
       ...
       y1[k] = 100.*(recoKE_keras[i]-trueKE3[i])/(1.*trueKE3[i]) 
       yTitus[k] = 100.*(recoKE_TMVAMLP[i]-trueKE2[i])/(1.*trueKE2[i])
       k=k+1
...
f1=ax.hist(y1, nbins, histtype='step', fill=False, edgecolor='fuchsia', hatch="/",alpha=0.75)#normed=1
f2=ax.hist(yTitus, nbins, histtype='step', fill=False, edgecolor='g', hatch="\\\\",alpha=0.75)
...
#ax.text(-95, 5300,'$E_{MC, muon}$: 200-600 MeV', bbox={'facecolor':'white','alpha': 0.2, 'pad': 5})
ax.text(-95, 780,'$E_{MC, muon}$: 600-1400 MeV', bbox={'facecolor':'white','alpha': 0.2, 'pad': 5})
### box filled
b_line = mpatches.Patch(fill=True, color='gold',label='Scikit BDT')
r_line = mpatches.Patch(fill=False, edgecolor='fuchsia', hatch="/", label='TNN')
bl_line = mpatches.Patch(fill=False, edgecolor='g', hatch="\\\\",label='MLPNN')
### solid line:
...
#plt.savefig("AINfidener_reso_ScikitBDTG_greenkeras_orangeTMVAMLP_newFid.png")
#plt.savefig("AINfidener_reso_ScikitBDTG_greenkeras_orangeTMVAMLPHE_newFid.png")
#plt.savefig("AINfidener_reso_ScikitBDTG_greenkeras_orangeTMVAMLP.png")
plt.savefig("AINfidener_reso_ScikitBDTG_greenkeras_orangeTMVAMLPHE.png")
