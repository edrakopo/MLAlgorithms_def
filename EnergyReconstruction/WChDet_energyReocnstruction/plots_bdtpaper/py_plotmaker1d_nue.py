...
rfile1 = ROOT.TFile('/Users/edrakopo/work/work_energy_reco/FINALTMVARegApp_BDTG_ener_lilia_CCQE_1040_1100_NEWlookupsTEST_MYselectedVars_nue.root')#TMVA BDTG/TITUS
...
recoDwall = arr_hi_E1['recoDWall']
recoToWall = arr_hi_E1['recoToWall']
...
#for i in range(0,10):
    if trueKE[i]>E_l and trueKE[i]<E_h and recoDwall[i]*550.>dwall_thres and recoToWall[i]*2500.>towall_thres:
       ...
       y1[k] = 100.*(recoKE_TMVABDTG[i]-trueKE2[i])/(1.*trueKE2[i])
...
print('entries: ', len(y), '/',len(yTitus) )
...
#f0=ax.hist(y, nbins, histtype='step', fill=False, edgecolor='black', hatch="|",alpha=0.75)
...
#ax.text(-95, 310,'$E_{MC, electron}$: 200-600 MeV', bbox={'facecolor':'white','alpha': 0.2, 'pad': 5})
ax.text(-95, 330,'$E_{MC, electron}$: 600-1400 MeV', bbox={'facecolor':'white','alpha': 0.2, 'pad': 5})
### box filled
...
bl_line = mpatches.Patch(fill=False, edgecolor='blue', hatch="\\\\",label='Lookup Tables')
### solid line:
...
plt.legend(handles=[b_line, bl_line])
#plt.legend(handles=[b_line, r_line])

#plt.savefig("energy_resol_electrons.png")
#plt.savefig("AINfidener_reso_ScikitBDTG_blueTITUS_nue.png")
plt.savefig("AINfidener_reso_ScikitBDTG_blueTITUS_nueHE.png")
#plt.savefig("AINfidener_reso_ScikitBDTG_blueTITUS_nue_newfid.png")
#plt.savefig("AINfidener_reso_ScikitBDTG_blueTITUS_nueHE_newfid.png")
